...
function dibujarBoletaAdiestramiento(canvas, emitida, logoImg, huellaImg) {
  ...
  ctx.fillStyle = "#8A7E5C";
  ctx.fillText(`Perro: ${emitida.perro || "—"}`, M, y + 21);
  ctx.fillText(`Servicio: Adiestramiento (${emitida.modalidad === "grupal" ? "grupal" : "individual"})`, M, y + 41);
  ...
  y += 36;
  const subtotalClases = emitida.numClases * emitida.precioClase;
  const montoDescuento = Math.round(subtotalClases * (emitida.descuentoPackPct / 100));
  const filas = [
  ...
  if (emitida.descuentoPackPct > 0) {
    filas.push({ texto: `Descuento pack de ${emitida.numClases} clases (-${emitida.descuentoPackPct}%)`, valor: `- ${fmtCLP(montoDescuento)}`, color: RUST });
  }
...
function BoletasAdiestramiento({ clientes, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  const montoDescuento = Math.round(subtotalClases * (Number(descuentoPackPct || 0) / 100));
  ...
  function elegirPack(n, descuento) {
    setNumClases(n);
    setDescuentoPackPct(descuento);
    setEmitida(null);
  }
  ...
  return (
    <div className="howria-split" style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 28 }}>
      <div className="howria-card" style={tarjeta}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
          <button type="button" onClick={() => elegirPack(4, 0)} style={botonSecundario}>4 clases — sin descuento</button>
        ...
        <div style={{ marginTop: 20, padding: "16px 18px", background: CREAM_SOFT, borderRadius: 8 }}>
          ...
          {montoDescuento > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: RUST, marginTop: 4 }}>
              <span>Descuento pack (-{descuentoPackPct}%)</span>
        ...
        <button onClick={generar} disabled={!cliente}
