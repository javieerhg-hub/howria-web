...
function dibujarCalendarioBoleta(ctx, x, yTop, mesIdx, anio, diasMarcados) {
  ...
  ctx.fillStyle = "#6B6248";
  ctx.font = "600 13px Helvetica";
  ctx.fillText(`${MESES[mesIdx].toUpperCase()} ${anio} — días de paseo contratados`, x, y);
  y += 26;
  ...
  celdas.forEach((d, i) => {
    ...
    ctx.fillStyle = marcado ? (recargo ? RUST : NAVY) : "#FFFFFF";
    ctx.strokeStyle = marcado ? (recargo ? "#7A3A24" : NAVY) : (recargo ? "#E3B7A8" : "#E4DBC3");
    ctx.lineWidth = marcado ? 2.5 : 1;
    ...
    ctx.fillStyle = marcado ? CREAM : (recargo ? "#C77A5E" : "#C4BCA0");
    ctx.font = marcado ? "700 17px Helvetica" : "13px Helvetica";
    ctx.textAlign = "center";
    ctx.fillText(String(d), cx + cellSize / 2, cy + (marcado ? cellSize / 2 + 1 : cellSize / 2 - 2));

    if (marcado) {
      ctx.font = "700 10px Helvetica";
      ctx.fillStyle = recargo ? "#FFE3D6" : GOLD;
      ctx.fillText(recargo ? "+30%" : "🐾 paseo", cx + cellSize / 2, cy + cellSize - 8);
    }
  });
  ctx.textAlign = "left";

  // leyenda
  const leyendaY = y + Math.ceil(celdas.length / 7) * (cellSize + gap) + 16;
  ctx.font = "12px Helvetica";
  roundRect(ctx, x, leyendaY, 16, 16, 4); ctx.fillStyle = NAVY; ctx.fill();
  ctx.fillStyle = "#5C5442"; ctx.textAlign = "left";
  ctx.font = "600 12px Helvetica";
  ctx.fillText("Día de paseo contratado", x + 22, leyendaY + 12);
  roundRect(ctx, x + 190, leyendaY, 16, 16, 4); ctx.fillStyle = RUST; ctx.fill();
  ctx.fillStyle = "#5C5442";
  ctx.fillText("Fin de semana / feriado (+30%)", x + 212, leyendaY + 12);
}
...
function IngresoPersonalNuevo({ clientes, setClientes, usuarios, setUsuarios }) {
  ...
  function registrar() {
    if (!nombre.trim() || clientesElegidos.length === 0) return;
    ...
    setConfirmacion(`${nuevoUsuario.nombre} quedó registrado con ${clientesElegidos.length} cliente(s) asignado(s). Para que pueda entrar, créale la cuenta en Supabase → Authentication → Add user, con el correo "${nuevoUsuario.email}" y la contraseña que quieras asignarle.`);
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <button onClick={registrar} disabled={!nombre.trim() || clientesElegidos.length === 0}
...
export default function HowriaAdmin() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (!data.session) setVerificandoSesion(false);
    });
  }, []);
