...
// Hook genérico: carga una tabla de Supabase y sincroniza cualquier
...
// ---------- Login ----------
function Login({ onLogin, clientes, onLoginCliente }) {
    ...
    <div style={{ minHeight: "100vh", background: NAVY, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 380, padding: "0 24px" }}>
          ...
          {modo === "equipo" ? (
            <>
              ...
              <input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Tu nombre"
                onKeyDown={(e) => e.key === "Enter" && nombre.trim() && onLogin({ nombre: nombre.trim(), rol })}
                style={input} autoFocus />
              <label style={label}>Rol</label>
              <select value={rol} onChange={(e) => setRol(e.target.value)} style={{ ...input, marginBottom: 24 }}>
                <option value="coordinador">Coordinador</option>
                <option value="entrenador">Entrenador</option>
                <option value="administrador">Administrador general</option>
              </select>
              <button onClick={() => nombre.trim() && onLogin({ nombre: nombre.trim(), rol })} style={botonPrincipal}>
                Entrar
              </button>
            </>
          ) : (
        ...
        <p style={{ fontSize: 12, color: "#7E8FA0", marginTop: 18, textAlign: "center", lineHeight: 1.5 }}>
          Prototipo — este login todavía no está conectado a la base de datos real.
        </p>
        <p style={{ fontSize: 12, color: "#7E8FA0", marginTop: 4, textAlign: "center", lineHeight: 1.5 }}>
          Para probar la vista del paseador, entra como <b>Pedro Vidal</b> con rol Entrenador.
        </p>
      </div>
    </div>
  );
...
function dibujarBoleta(canvas, emitida, logoImg) {
  ...
  const diasTexto = emitida.dias.join(" · ");
  wrapText(ctx, diasTexto, 56, y, W - 112, 20);
...
function IngresoPersonalNuevo({ clientes, setClientes, usuarios, setUsuarios }) {
  ...
  function registrar() {
    ...
    const nuevoUsuario = { id: Date.now(), nombre: nombre.trim(), rol, fotoUrl, fechaInicio, datosBancarios: { banco, tipoCuenta, numeroCuenta } };
    setUsuarios((prev) => [...prev, nuevoUsuario]);
    setClientes((prev) => prev.map((c) => (seleccionados.includes(c.id) ? { ...c, paseadorNombre: nuevoUsuario.nombre } : c)));
    setConfirmacion(`${nuevoUsuario.nombre} quedó registrado con ${clientesElegidos.length} cliente(s) asignado(s).`);
...
export default function HowriaAdmin() {
  ...
  if (!user) return <Login clientes={clientes} onLoginCliente={setClienteSesion} onLogin={(u) => { setUser(u); if (u.rol === "entrenador") setTab("mis-paseos"); }} />;
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM_SOFT, fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div className="howria-main" style={{ padding: "28px 32px", maxWidth: 1040, margin: "0 auto" }}>
        ...
        {tab === "equipo" && puedeVerFinanzas && <EquipoTrabajo equipo={equipoInterno} setEquipo={setEquipoInterno} objetivos={objetivosSemanales} setObjetivos={setObjetivosSemanales} tareas={tareasEquipo} setTareas={setTareasEquipo} />}
