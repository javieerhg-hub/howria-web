...
function dibujarBoleta(canvas, emitida, logoImg, huellaImg) {
  ...
  // marca de agua sutil: la huellita real, repetida sobre el cuerpo blanco
  if (huellaImg) {
    ctx.save();
    ctx.globalAlpha = 0.09;
    const hW = 34, hH = 34 * (huellaImg.height / huellaImg.width);
    const pasoX = 74, pasoY = 88;
    let fila = 0;
    for (let wy = 150; wy < H - 20; wy += pasoY) {
      const desfase = (fila % 2 === 0) ? 0 : pasoX / 2;
      for (let wx = -pasoX; wx < W + pasoX; wx += pasoX) {
        ctx.drawImage(huellaImg, wx + desfase - hW / 2, wy - hH / 2, hW, hH);
      }
      fila++;
    }
    ctx.restore();
