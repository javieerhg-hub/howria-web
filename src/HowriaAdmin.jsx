        ...
        {celdas.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => onToggle(d)} type="button"
              style={{
                aspectRatio: "1", border: activo ? `1.5px solid ${GOLD}` : "1px solid #E4DBC3",
                background: activo ? NAVY : "#FFFFFF", borderRadius: 6, cursor: "pointer",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 13, color: activo ? CREAM : INK, fontWeight: activo ? 600 : 400,
              }}>
              {d}
            </button>
          );
...
function dibujarBoleta(canvas, emitida, logoImg) {
  ...
  y += 46;
  const filas = [
    [`Paseos realizados (${emitida.cantidad} x ${fmtCLP(emitida.valorPaseo)})`, fmtCLP(emitida.cantidad * emitida.valorPaseo)],
  ];
...
function dibujarCalendarioBoleta(ctx, x, yTop, mesIdx, anio, diasMarcados) {
  ...
  celdas.forEach((d, i) => {
    ...
    ctx.fillStyle = marcado ? NAVY : "#F5F1E4";
    ctx.strokeStyle = marcado ? GOLD : "#E4DBC3";
    ctx.lineWidth = marcado ? 2 : 1;
    roundRect(ctx, cx, cy, cellSize, cellSize, 6);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = marcado ? CREAM : "#B0A587";
    ctx.font = marcado ? "600 14px Helvetica" : "14px Helvetica";
    ctx.textAlign = "center";
...
function Boletas({ clientes, boletasEmitidas, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  const subtotal = dias.length * Number(valorPaseo || 0);
  ...
  return (
    <div className="howria-split" style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 28 }}>
      <div className="howria-card" style={tarjeta}>
        ...
        <p style={hint}>El plan marca los días automáticamente en el calendario — puedes ajustarlos a mano si algún día cambia.</p>
        ...
        <div style={{ marginTop: 8, padding: "16px 18px", background: CREAM_SOFT, borderRadius: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: INK }}>
            <span>{dias.length} paseos × {fmtCLP(valorPaseo)}</span>
