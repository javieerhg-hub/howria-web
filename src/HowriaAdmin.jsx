...
function useSyncedTable(tableName, mapToDb, mapFromDb, orderBy) {
    ...
    setRegistroState((prev) => {
      ...
      Object.keys(next).forEach((key) => {
        if (JSON.stringify(next[key]) !== JSON.stringify(prev[key])) {
          ...
          const r = next[key];
          const estado = r.cancelado ? "cancelado" : r.realizado ? "realizado" : null;
          if (estado) {
            supabase.from("registro_paseos").upsert(
              { cliente_id: cliente._dbId, fecha, estado, nota: r.nota || null },
...
function Login({ onLogin, clientes, usuarios, onLoginCliente }) {
  ...
  function intentarLogin() {
    const encontrado = usuarios.find(
      (u) => u.nombre.trim().toLowerCase() === nombre.trim().toLowerCase() && u.password === passwordEquipo
    );
    if (encontrado) {
      setErrorLogin("");
      onLogin(encontrado);
        ...
        <div style={{ background: CREAM, borderRadius: 10, padding: "36px 32px", boxShadow: "0 24px 60px rgba(0,0,0,0.35)" }}>
          ...
          {modo === "equipo" ? (
            <>
              ...
              <button onClick={intentarLogin} style={botonPrincipal}>
                Entrar
              </button>
            </>
          ) : (
...
function PortalCliente({ cliente, boletasCliente, onSalir }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM_SOFT, fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
...
function dibujarBoleta(canvas, emitida, logoImg) {
  ...
  if (logoImg) {
    const logoH = 110, logoW = logoImg.width * (logoH / logoImg.height);
    ctx.drawImage(logoImg, 40, 30, logoW, logoH);
  }
  ctx.fillStyle = CREAM;
  ctx.font = "600 26px Georgia";
  ctx.textAlign = "right";
  ctx.fillText(`Boleta N°${String(emitida.numero).padStart(3, "0")}`, W - 40, 75);
  ctx.font = "15px Helvetica";
  ctx.fillText(`Emitida el ${emitida.fecha}`, W - 40, 105);
  ...
  ctx.fillText(emitida.cliente, 40, y);
  ctx.font = "16px Helvetica";
  ctx.fillStyle = "#8A7E5C";
  ctx.fillText(`Perro: ${emitida.perro}`, 40, y + 26);
  ctx.fillText(`Servicio: Paseos de ${emitida.mes} ${emitida.anio}`, 40, y + 50);
  ctx.fillText(`Plan: ${emitida.planNombre}`, 40, y + 74);

  // tabla detalle
  y += 120;
  ctx.fillStyle = CREAM_SOFT;
  ctx.fillRect(40, y, W - 80, 46);
  ctx.fillStyle = "#6B6248";
  ctx.font = "600 13px Helvetica";
  ctx.fillText("DETALLE", 56, y + 28);
  ctx.textAlign = "right";
  ctx.fillText("VALOR", W - 56, y + 28);
  ctx.textAlign = "left";
  ...
  const filas = [
    [`Paseos día hábil (${diasNormalesEmitida.length} x ${fmtCLP(emitida.valorPaseo)})`, fmtCLP(diasNormalesEmitida.length * emitida.valorPaseo)],
  ];
  if (diasRecargoEmitida.length > 0) {
    filas.push([`Paseos fin de semana / feriado +30% (${diasRecargoEmitida.length} x ${fmtCLP(valorConRecargo(emitida.valorPaseo, true))})`, fmtCLP(diasRecargoEmitida.length * valorConRecargo(emitida.valorPaseo, true))]);
  }
  if (emitida.paseosCancelados > 0) {
    filas.push([`Descuento paseos cancelados mes anterior (${emitida.paseosCancelados} x ${fmtCLP(emitida.valorPaseo)})`, `- ${fmtCLP(emitida.descuento)}`]);
  }
  filas.forEach(([texto, valor], idx) => {
    const fy = y + 40 * idx + 28;
    ctx.strokeStyle = "#EDE4CE";
    ctx.beginPath(); ctx.moveTo(40, y + 40 * idx); ctx.lineTo(W - 40, y + 40 * idx); ctx.stroke();
    ctx.fillStyle = INK;
    ctx.font = "15px Helvetica";
    ctx.fillText(texto, 56, fy, 560);
    ctx.textAlign = "right";
    ctx.fillStyle = idx > 0 ? RUST : INK;
    ctx.fillText(valor, W - 56, fy);
    ctx.textAlign = "left";
  });
  y += 40 * filas.length + 20;
  ...
  ctx.fillStyle = "#8A7E5C";
  ctx.font = "600 13px Helvetica";
  ctx.fillText("DÍAS DE PASEO RESERVADOS", 56, y);
  y += 26;
  ctx.fillStyle = INK;
  ctx.font = "14px Helvetica";
  dibujarCalendarioBoleta(ctx, 56, y, MESES.indexOf(emitida.mes), emitida.anio, emitida.dias);
  ...
  ctx.fillText("TOTAL A PAGAR", 60, y + 34);
  ctx.font = "700 30px Georgia";
  ctx.textAlign = "right";
  ctx.fillText(fmtCLP(emitida.total), W - 60, y + 52);
  ctx.textAlign = "left";
  ...
  ctx.fillText("Gracias por confiar en Howria", W / 2, H - 40);
  ctx.textAlign = "left";
}

function dibujarCalendarioBoleta(ctx, x, yTop, mesIdx, anio, diasMarcados) {
  const total = diasDelMes(mesIdx, anio);
  const primerDia = new Date(anio, mesIdx, 1).getDay();
  const offset = (primerDia + 6) % 7;
  const celdas = [];
  ...
  ctx.fillStyle = "#9A9179";
  DIAS_SEMANA.forEach((d, i) => {
    ctx.fillText(d, x + i * (cellSize + gap) + cellSize / 2, y + 10);
  });
  y += 22;
  ...
  celdas.forEach((d, i) => {
    ...
    const cy = y + fila * (cellSize + gap);
    if (d === null) return;
    const marcado = diasMarcados.includes(d);
    const recargo = esFinDeSemanaOFeriado(new Date(anio, mesIdx, d));
    ctx.fillStyle = marcado ? (recargo ? RUST : NAVY) : (recargo ? "#F1DCD2" : "#F5F1E4");
    ctx.strokeStyle = marcado ? (recargo ? "#7A3A24" : GOLD) : (recargo ? RUST : "#E4DBC3");
    ctx.lineWidth = marcado ? 2 : 1;
    roundRect(ctx, cx, cy, cellSize, cellSize, 6);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = marcado ? CREAM : (recargo ? RUST : "#B0A587");
    ctx.font = marcado || recargo ? "600 14px Helvetica" : "14px Helvetica";
    ctx.textAlign = "center";
    ctx.fillText(String(d), cx + cellSize / 2, cy + cellSize / 2 + 5);
  });
  ctx.textAlign = "left";

  // leyenda
  const leyendaY = y + Math.ceil(celdas.length / 7) * (cellSize + gap) + 14;
  ctx.font = "12px Helvetica";
  roundRect(ctx, x, leyendaY, 14, 14, 3); ctx.fillStyle = NAVY; ctx.fill();
  ctx.fillStyle = "#6B6248"; ctx.textAlign = "left";
  ctx.fillText("Día contratado", x + 20, leyendaY + 11);
  roundRect(ctx, x + 150, leyendaY, 14, 14, 3); ctx.fillStyle = RUST; ctx.fill();
  ctx.fillStyle = "#6B6248";
  ctx.fillText("Fin de semana / feriado (+30%)", x + 170, leyendaY + 11);
}
...
function Boletas({ clientes, boletasEmitidas, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  function imprimirPDF() {
    ...
    ventana.document.write(`<img src="${dataUrl}" style="width:100%" onload="window.print()" />`);
  ...
  function enviarWhatsapp() {
    ...
    const mensaje = encodeURIComponent(
      `Hola ${cliente.nombre.split(" ")[0]}! Te comparto la boleta N°${String(emitida.numero).padStart(3, "0")} de ${emitida.mes} ${emitida.anio} — ${emitida.cantidad} paseos, total ${fmtCLP(emitida.total)}. Te adjunto la imagen a continuación. ¡Gracias por confiar en Howria! 🐾`
    );
...
function Clientes({ clientes, setClientes, boletasEmitidas, usuarios, puedeEliminar }) {
  ...
  if (clientePerfil) {
    return (
      <PerfilCliente
        ...
        boletasCliente={boletasEmitidas.filter((b) => b.cliente === clientePerfil.nombre)}
  ...
  const filtrados = clientes
    .filter((c) => {
      ...
      if (!q) return true;
      return c.nombre.toLowerCase().includes(q) || c.perro.toLowerCase().includes(q);
    })
  ...
  return (
    <div className="howria-card" style={tarjeta}>
      ...
      <input placeholder="Buscar por cliente o perro..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)}
        style={{ ...input, marginTop: 18, marginBottom: 4, maxWidth: 340 }} />
...
function Finanzas({ boletasEmitidas, clientes }) {
  ...
  const clientesSinBoletaEsteMes = useMemo(() => {
    return clientes.filter((c) => !boletasEmitidas.some((b) => {
      const f = new Date(b.fechaISO);
      return b.cliente === c.nombre && f.getMonth() === hoy.getMonth() && f.getFullYear() === hoy.getFullYear();
    }));
  ...
  return (
    <div className="howria-card" style={tarjeta} id="reporte-finanzas">
      ...
      <div className="howria-g4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 26 }}>
        ...
        <div style={{ background: CREAM_SOFT, borderRadius: 10, padding: 18 }}>
          <p style={{ margin: "0 0 6px", fontSize: 12, color: "#8A7E5C", textTransform: "uppercase", letterSpacing: 0.5 }}>Descuentos por cancelación</p>
          <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: RUST, fontFamily: "Georgia, serif" }}>{fmtCLP(actual.descuentos)}</p>
        </div>
      </div>
...
function PanelAdmin({ usuarios, setUsuarios }) {
  const [nuevo, setNuevo] = useState({ nombre: "", rol: "coordinador" });

  return (
    <div className="howria-card" style={tarjeta}>
      <h2 style={sectionTitle}>Usuarios del sistema</h2>
      <div style={{ marginBottom: 20 }}>
        {usuarios.map((u) => (
          <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#FFFFFF", border: "1px solid #E4DBC3", borderRadius: 8, marginBottom: 8 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 10, color: NAVY }}>
              <span style={{ width: 30, height: 30, borderRadius: "50%", background: u.fotoUrl ? `url(${u.fotoUrl}) center/cover` : CREAM_SOFT, display: "inline-block" }} />
              {u.nombre}
            </span>
            <span style={{ color: "#8A7E5C", fontSize: 13, textTransform: "capitalize" }}>{u.rol}</span>
            <button onClick={() => setUsuarios((prev) => prev.filter((x) => x.id !== u.id))} style={{ border: "none", background: "none", color: RUST, cursor: "pointer", fontSize: 12 }}>Quitar</button>
          </div>
        ))}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <input placeholder="Nombre" value={nuevo.nombre} onChange={(e) => setNuevo({ ...nuevo, nombre: e.target.value })} style={{ ...input, flex: 1, marginBottom: 0 }} />
        <select value={nuevo.rol} onChange={(e) => setNuevo({ ...nuevo, rol: e.target.value })} style={{ ...input, marginBottom: 0, width: 180 }}>
          <option value="coordinador">Coordinador</option>
          <option value="entrenador">Entrenador</option>
          <option value="administrador">Administrador general</option>
        </select>
        <button onClick={() => { if (nuevo.nombre) { setUsuarios((p) => [...p, { id: Date.now(), ...nuevo }]); setNuevo({ nombre: "", rol: "coordinador" }); } }} style={{ ...botonPrincipal, width: "auto", padding: "0 22px" }}>Agregar</button>
      </div>
    </div>
  );
}
...
function calcularAvisos({ clientes, boletasEmitidas, registroPaseos, tareasEquipo, citasAgenda = [], prospectos = [] }) {
  ...
  if (pendientes.length > 0) {
    avisos.push({ tipo: "factura", icono: "💰", texto: `${pendientes.length} boleta(s) por cobrar — ${fmtCLP(montoPendiente)}` });
  }
  ...
  if (sinMarcar.length > 0) {
    avisos.push({ tipo: "paseo", icono: "🐾", texto: `${sinMarcar.length} paseo(s) de hoy sin marcar como realizado`, detalle: sinMarcar.map((c) => `${c.nombre} (${c.paseadorNombre || "sin paseador"})`).join(", ") });
  }

  const tareasHoy = tareasEquipo.filter((t) => fechaKey(new Date(t.fechaISO)) === fechaKey(hoy) && t.estado !== "hecho");
  if (tareasHoy.length > 0) {
    avisos.push({ tipo: "tarea", icono: "📋", texto: `${tareasHoy.length} tarea(s) del equipo pendiente(s) para hoy` });
  }

  const sinPaseador = clientes.filter((c) => !c.paseadorNombre);
  if (sinPaseador.length > 0) {
    avisos.push({ tipo: "asignacion", icono: "⚠️", texto: `${sinPaseador.length} cliente(s) sin paseador asignado` });
  }
  ...
  if (necesitanEvaluacion.length > 0) {
    avisos.push({ tipo: "evaluacion", icono: "📅", texto: `${necesitanEvaluacion.length} cliente(s) con evaluación pendiente de agendar` });
  }
  ...
  if (prospectosVencidos.length > 0) {
    avisos.push({ tipo: "prospecto", icono: "📞", texto: `${prospectosVencidos.length} prospecto(s) con seguimiento pendiente` });
  }
...
function realizadosEnRango(registroPaseos, clienteId, desde, hasta) {
  let n = 0;
  const cur = new Date(desde);
  while (cur < hasta) {
    if (registroPaseos[`${clienteId}_${fechaKey(cur)}`]?.realizado) n++;
    cur.setDate(cur.getDate() + 1);
  }
  return n;
}
...
function PagoTrabajadores({ boletasEmitidas, clientes, usuarios, registroPaseos, pagosRegistrados, setPagosRegistrados }) {
  ...
  const resumenPorPaseador = useMemo(() => {
    ...
    const nombresConocidos = new Set([
      ...usuarios.filter((u) => u.rol === "entrenador" || u.rol === "coordinador").map((u) => u.nombre),
    ...
    clientes.filter((c) => c.paseadorNombre).forEach((c) => {
      ...
      const realizadosRaw = realizadosEnRango(registroPaseos, c.id, desde, hasta);
      ...
      const facturaMes = boletasEmitidas.find((b) => b.cliente === c.nombre && b.mes === MESES[mesActual] && b.anio === anioActual);
...
function Asignaciones({ clientes, setClientes, usuarios }) {
  const paseadoresDisponibles = usuarios.filter((u) => u.rol === "entrenador" || u.rol === "coordinador");
...
function MapaRutas({ clientes, setClientes, usuarios }) {
  const paseadores = usuarios.filter((u) => u.rol === "entrenador" || u.rol === "coordinador");
...
function IngresoPersonalNuevo({ clientes, setClientes, usuarios, setUsuarios }) {
  ...
  function registrar() {
    if (!nombre.trim() || !password.trim() || clientesElegidos.length === 0) return;
    const nuevoUsuario = { id: Date.now(), nombre: nombre.trim(), rol, fotoUrl, fechaInicio, password: password.trim(), datosBancarios: { banco, tipoCuenta, numeroCuenta } };
    setUsuarios((prev) => [...prev, nuevoUsuario]);
    setClientes((prev) => prev.map((c) => (seleccionados.includes(c.id) ? { ...c, paseadorNombre: nuevoUsuario.nombre } : c)));
    setConfirmacion(`${nuevoUsuario.nombre} quedó registrado con ${clientesElegidos.length} cliente(s) asignado(s). Contraseña: ${nuevoUsuario.password}`);
    setNombre(""); setFotoUrl(null); setSeleccionados([]); setRol("entrenador"); setFechaInicio(""); setBanco(""); setNumeroCuenta(""); setPassword("");
  }
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div className="howria-card" style={tarjeta}>
        ...
        <input placeholder="Contraseña para iniciar sesión" value={password} onChange={(e) => setPassword(e.target.value)} style={{ ...input, maxWidth: 320 }} />

        <p style={{ ...label, marginTop: 6 }}>Datos bancarios para el pago</p>
      ...
      <button onClick={registrar} disabled={!nombre.trim() || !password.trim() || clientesElegidos.length === 0}
...
function Inicio({ clientes, boletasEmitidas, registroPaseos, tareasEquipo, objetivosSemanales, usuarios, citasAgenda, prospectos, setTab, user }) {
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
  const dow = (hoy.getDay() + 6) % 7;
  const avisos = calcularAvisos({ clientes, boletasEmitidas, registroPaseos, tareasEquipo, citasAgenda, prospectos });
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div className="howria-card" style={tarjeta}>
        <h2 style={sectionTitle}>Hola, {user.nombre.split(" ")[0]}</h2>
        <p style={{ ...hint, textTransform: "capitalize" }}>{fechaLarga}</p>
      </div>
      ...
      {avisos.length > 0 && (
        <div className="howria-card" style={{ ...tarjeta, background: "#F3E3B4", border: "1px solid #E3D08C" }}>
          ...
          {avisos.map((a, i) => (
            <p key={i} style={{ margin: "0 0 6px", fontSize: 13.5, color: "#6B5518" }}>{a.icono} {a.texto}</p>
          ))}
        </div>
      )}
...
// ---------- Estilos compartidos ----------
const sectionTitle = { fontSize: 17, color: NAVY, fontFamily: "'Baskerville', 'Georgia', serif", marginBottom: 6, marginTop: 0 };
const hint = { fontSize: 12.5, color: "#9A9179", margin: "0 0 12px" };
const label = { display: "block", fontSize: 12, color: "#8A7E5C", marginBottom: 6, fontWeight: 600, letterSpacing: 0.3 };
const input = { width: "100%", boxSizing: "border-box", padding: "10px 12px", marginBottom: 16, border: "1px solid #DCD2B4", borderRadius: 6, fontSize: 14, background: "#FFFFFF", fontFamily: "'Helvetica Neue', Arial, sans-serif", color: INK };
const botonPrincipal = { width: "100%", padding: "12px", background: NAVY, color: CREAM, border: "none", borderRadius: 6, fontSize: 14, cursor: "pointer", fontWeight: 600, letterSpacing: 0.3 };
const botonSecundario = { padding: "10px 18px", background: "transparent", color: NAVY, border: `1.5px solid ${NAVY}`, borderRadius: 6, fontSize: 13, cursor: "pointer", fontWeight: 600, flex: 1 };
const tarjeta = { background: "#FFFFFF", border: "1px solid #EDE4CE", borderRadius: 12, padding: 24 };
...
const EQUIPO_INICIAL = ["Javier Arunías", "Javier Herrera", "Oliska Mendoza"];

// ---------- App ----------
export default function HowriaAdmin() {
  const [user, setUser] = useState(null);
  const [tab, setTab] = useState("inicio");
  const [clientes, setClientes] = useSyncedTable("clientes", clienteToDb, dbToCliente, "nombre");
  const [correlativo, setCorrelativo] = useState(107);
  const [boletasEmitidas, setBoletasEmitidas] = useSyncedTable("boletas", boletaToDb, dbToBoleta, "numero");
  const [usuarios, setUsuarios] = useSyncedTable("usuarios", usuarioToDb, dbToUsuario, "nombre");
  ...
  if (clienteSesion) {
    return (
      <PortalCliente
        ...
        boletasCliente={boletasEmitidas.filter((b) => b.cliente === clienteSesion.nombre)}
  ...
  const tabs = esPaseador
    ? [{ id: "mis-paseos", label: "Mis paseos" }]
    : [
        ...
        ...(puedeVerFinanzas ? [{ id: "finanzas", label: "Finanzas" }] : []),
        ...(puedeVerFinanzas ? [{ id: "pagos", label: "Pago trabajadores" }] : []),
        ...(puedeVerFinanzas ? [{ id: "asignaciones", label: "Asignaciones" }] : []),
        ...(puedeVerFinanzas ? [{ id: "mapa", label: "Mapa" }] : []),
        ...(esAdmin ? [{ id: "ingreso-personal", label: "Ingreso personal nuevo" }] : []),
        ...(puedeVerFinanzas ? [{ id: "equipo", label: "Equipo" }] : []),
        ...(puedeVerFinanzas ? [{ id: "agenda", label: "Agenda" }] : []),
        ...(puedeVerFinanzas ? [{ id: "seguimiento", label: "Seguimiento" }] : []),
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM_SOFT, fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      `}</style>
      <div className="howria-header" style={{ background: NAVY, padding: "14px 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <div className="howria-tabs-desktop" style={{ display: "flex", gap: 4, padding: "0 32px", background: "#FFFFFF", borderBottom: "1px solid #EDE4CE", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{
              padding: "16px 20px", border: "none", background: "none", cursor: "pointer",
              fontSize: 14, color: tab === t.id ? NAVY : "#B0A587", flex: "none",
      ...
      <div className="howria-main" style={{ padding: "28px 32px", maxWidth: 1040, margin: "0 auto" }}>
        {tab === "inicio" && !esPaseador && <Inicio clientes={clientes} boletasEmitidas={boletasEmitidas} registroPaseos={registroPaseos} tareasEquipo={tareasEquipo} objetivosSemanales={objetivosSemanales} usuarios={usuarios} citasAgenda={citasAgenda} prospectos={prospectos} setTab={setTab} user={user} />}
        {tab === "mis-paseos" && <MisPaseos clientes={clientes} registroPaseos={registroPaseos} setRegistroPaseos={setRegistroPaseos} user={user} />}
        {tab === "boletas" && <Boletas clientes={clientes} boletasEmitidas={boletasEmitidas} correlativo={correlativo} setCorrelativo={setCorrelativo} onRegistrarBoleta={(b) => setBoletasEmitidas((prev) => [...prev, b])} />}
        {tab === "facturas" && <Facturas boletasEmitidas={boletasEmitidas} setBoletasEmitidas={setBoletasEmitidas} clientes={clientes} />}
        {tab === "clientes" && <Clientes clientes={clientes} setClientes={setClientes} boletasEmitidas={boletasEmitidas} usuarios={usuarios} puedeEliminar={esAdmin} />}
        {tab === "finanzas" && puedeVerFinanzas && <Finanzas boletasEmitidas={boletasEmitidas} clientes={clientes} />}
        {tab === "pagos" && puedeVerFinanzas && <PagoTrabajadores boletasEmitidas={boletasEmitidas} clientes={clientes} usuarios={usuarios} registroPaseos={registroPaseos} pagosRegistrados={pagosRegistrados} setPagosRegistrados={setPagosRegistrados} />}
        {tab === "asignaciones" && puedeVerFinanzas && <Asignaciones clientes={clientes} setClientes={setClientes} usuarios={usuarios} />}
        {tab === "mapa" && puedeVerFinanzas && <MapaRutas clientes={clientes} setClientes={setClientes} usuarios={usuarios} />}
        {tab === "ingreso-personal" && esAdmin && <IngresoPersonalNuevo clientes={clientes} setClientes={setClientes} usuarios={usuarios} setUsuarios={setUsuarios} />}
        {tab === "equipo" && puedeVerFinanzas && <EquipoTrabajo equipo={equipoInterno} setEquipo={setEquipoInterno} objetivos={objetivosSemanales} setObjetivos={setObjetivosSemanales} objetivosMensuales={objetivosMensuales} setObjetivosMensuales={setObjetivosMensuales} tareas={tareasEquipo} setTareas={setTareasEquipo} />}
        {tab === "agenda" && puedeVerFinanzas && <Agenda clientes={clientes} usuarios={usuarios} citas={citasAgenda} setCitas={setCitasAgenda} />}
        {tab === "seguimiento" && puedeVerFinanzas && <Prospectos prospectos={prospectos} setProspectos={setProspectos} setClientes={setClientes} />}
        {tab === "usuarios" && esAdmin && <PanelAdmin usuarios={usuarios} setUsuarios={setUsuarios} />}
      </div>
    </div>
  );
}
