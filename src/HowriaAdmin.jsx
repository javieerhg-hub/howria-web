...
function dibujarBoletaAdiestramiento(canvas, emitida, logoImg, huellaImg) {
  ...
  const montoDescuentoPct = Math.round(subtotalClases * (emitida.descuentoPackPct / 100));
  const montoDescuento = montoDescuentoPct + (emitida.descuentoPackMonto || 0);
  const filas = [
...
function BoletasAdiestramiento({ clientes, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  const cliente = clienteManual
    ? { nombre: nombreManual.trim(), perro: "", telefono: "", _dbId: null }
