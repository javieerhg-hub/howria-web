...
function dibujarBoleta(canvas, emitida, logoImg) {
  ...
  // marca de agua sutil del logo, repetida sobre el cuerpo blanco
  if (logoImg) {
    ctx.save();
    ctx.globalAlpha = 0.035;
    const wmH = 130, wmW = logoImg.width * (wmH / logoImg.height);
    for (let wy = 150; wy < H - 20; wy += wmH + 46) {
      for (let wx = -40; wx < W; wx += wmW + 46) {
        ctx.drawImage(logoImg, wx, wy, wmW, wmH);
      }
    }
    ctx.restore();
  }
  ...
  ctx.textAlign = "left";
  ctx.fillStyle = INK;
  ctx.font = "600 17px 'Fraunces', Georgia";
  ...
  if (emitida.mensajePersonalizado) {
    ctx.font = "italic 12.5px 'Inter', Helvetica";
    ctx.fillStyle = GOLD;
    wrapTextInline(ctx, `“${emitida.mensajePersonalizado}”`, M, y + 80, W - M * 2);
    alturaExtraMensaje = 36;
  }

  // tabla detalle
  y += 92 + alturaExtraMensaje;
  ctx.fillStyle = CREAM_SOFT;
  ctx.fillRect(M, y, W - M * 2, 36);
  ctx.fillStyle = "#6B6248";
  ctx.font = "600 11px 'Inter', Helvetica";
  ...
  y += 24;
  const alturaCalendario = dibujarCalendarioBoleta(ctx, M, y, W - M * 2, MESES.indexOf(emitida.mes), emitida.anio, emitida.dias);
  ...
  ctx.font = "700 23px 'Fraunces', Georgia";
  ctx.textAlign = "right";
...
function Boletas({ clientes, boletasEmitidas, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  useEffect(() => {
    if (emitida && canvasRef.current) {
      const dibujar = () => dibujarBoleta(canvasRef.current, emitida, logoImgRef.current);
  ...
  return (
    <div className="howria-split" style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 28 }}>
      <div className="howria-card" style={tarjeta}>
        ...
        <label style={label}>Mensaje personalizado para el tutor (opcional)</label>
        <input type="text" placeholder="ej. ¡Gracias por otro mes con nosotros!" value={mensajePersonalizado} onChange={(e) => { setMensajePersonalizado(e.target.value); setEmitida(null); }} style={input} />
