...
function Boletas({ clientes, boletasEmitidas, correlativo, setCorrelativo, onRegistrarBoleta }) {
  ...
  async function enviarWhatsapp() {
    ...
    const mensaje = `Hola ${cliente.nombre.split(" ")[0]}! Te comparto la boleta N°${String(emitida.numero).padStart(3, "0")} de ${emitida.mes} ${emitida.anio} — ${emitida.cantidad} paseos, total ${fmtCLP(emitida.total)}. ¡Gracias por confiar en Howria! 🐾`;
    const nombreArchivo = `Boleta-${String(emitida.numero).padStart(3, "0")}-${emitida.cliente.replace(/\s+/g, "-")}.png`;

    // en el celular: comparte la imagen ya adjunta y deja elegir el contacto (WhatsApp u otro)
    ...
    // así que descargamos la imagen y abrimos el chat con el mensaje listo para adjuntarla a mano
    descargarPNG();
    const numero = (cliente.telefono || "").replace(/\D/g, "");
    window.open(`https://wa.me/${numero}?text=${encodeURIComponent(mensaje + " (adjunta la imagen que se acaba de descargar)")}`, "_blank");
  }
