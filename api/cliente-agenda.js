...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (clienteErr || !cliente) {
      res.status(404).json({ error: "Link inválido — no encontramos tu ficha de cliente", _debug: clienteErr ? clienteErr.message : "sin fila", _url: supabaseUrl, _hasKey: !!serviceRoleKey, _keyLen: (serviceRoleKey || "").length });
      return;
    }
